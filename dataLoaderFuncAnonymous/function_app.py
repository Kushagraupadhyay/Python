...
@app.route(route="http_trigger_data_loader_to_db")
def http_trigger_data_loader_to_db(req: func.HttpRequest) -> func.HttpResponse:

    logging.info('Python HTTP trigger function processing a request.')

    email_data = req.get_body().decode('utf-8')
    #email_data = email_data.replace('//', '__') # replacing // with __ to avoid error in reading html content
    logging.info(f'HTML content received: {email_data}')
    ...
    logging.info(f'HTML content received: {all_tables}')
    ...
    all_tables.loc[all_tables['TimeSinceLastReview']>10000, 'TimeSinceLastReview'] = 0 # setting the value to 0 if the value is greater than 10000
    ...
    all_tables['ReviewNeeded'] = all_tables['TimeSinceLastReview'].apply(lambda x: 'Yes' if x>10 else 'No') 
    ...
    return func.HttpResponse( 
            ...
            "Data was successfully pushed to DB" ,
            status_code=200 
        )
